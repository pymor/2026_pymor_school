...
- Vectors in pyMOR are just `VectorArrays` of length one.
  <br><span style="font-size: x-small">(`ListVectorArray` manages a list of `Vector` objects, but that's just an implementation detail.)</span>
...
<span style="font-size:x-small">(This is an implementation detail. There is no guarantee that `a` and `V` hold the same data.)</span>
...
- Consequence: there is no `transpose` method for `VectorArrays`.
  <br><span style="font-size:x-small">(Technical reason: transposing a list of MPI-distributed PETsc vectors would be expensive/unfeasible.)</span>
